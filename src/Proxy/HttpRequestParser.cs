...
internal class HttpRequestParser
{
    ...
    public static Dictionary<string, object> ParseCookies(IRequestCookieCollection requestCookies)
    {
        ...
        foreach (var cookie in requestCookies) cookies.Add(cookie.Key, string.Join(',', cookie.Value));
    ...
    public static Dictionary<string, object> ParseHeaders(IHeaderDictionary requestHeaders)
    {
        ...
        foreach (var header in requestHeaders) headers.Add(header.Key, string.Join(',', header.Value.ToString()));
    ...
    public static Dictionary<string, object> ParseQueryParams(IQueryCollection requestQuery)
    {
        ...
        foreach (var param in requestQuery) queryParams.Add(param.Key, string.Join(',', param.Value.ToString()));
