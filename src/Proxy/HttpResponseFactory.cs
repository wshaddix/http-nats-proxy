...
internal class HttpResponseFactory
{
        ...
        MicroserviceMessage message, JsonSerializerSettings jsonSerializerSettings)
    {
        ...
        foreach (var header in message.ResponseHeaders) httpResponse.GetTypedHeaders().Append(header.Key, header.Value);
        ...
        if (!string.IsNullOrWhiteSpace(message.ResponseBody)) await httpResponse.WriteAsync(message.ResponseBody).ConfigureAwait(false);
    }
}