...
internal class PipelineExecutor
{
    ...
    private static MicroserviceMessage ExtractMessageFromReply(Msg reply)
    {
        ...
        if (message is null) throw new Exception("The NATS reply message is not a valid MicroserviceMessage");
    ...
    private static void MergeMessageProperties(MicroserviceMessage message, MicroserviceMessage responseMessage)
    {
        ...
        {
            if (!message.ExtendedProperties.ContainsKey(h.Key)) message.ExtendedProperties.Add(h.Key, h.Value);
        });
        ...
        {
            if (!message.RequestHeaders.ContainsKey(h.Key)) message.RequestHeaders.Add(h.Key, h.Value);
        });
        ...
        {
            if (!message.ResponseHeaders.ContainsKey(h.Key)) message.ResponseHeaders.Add(h.Key, h.Value);
        });
    }
    ...
    private async Task ExecutePipelineInternalAsync(MicroserviceMessage message, Pipeline pipeline, bool allowTermination = true)
    {
        ...
        foreach (var step in pipeline.Steps)
        {
            ...
            if (allowTermination && message.ShouldTerminateRequest) break;
        }
    }
